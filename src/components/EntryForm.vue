        ...
        <div class="row row-height">
          ...
          <div class="col-height col-md-3">
            ...
            <div class="input-group has-validation" :class="{'has-error': v$.effecttime.$error}">
              <InputMask ref="effecttime" v-model="localData.effecttime" id="effecttime" name="effecttime" picture="99:99:99" /> 
              <label class="required">*</label>
            </div>
...
import { DEFAULT_CONTENT_TYPE, getApiUrl, disableControls }  from '@willsofts/will-app5';
import { startWaiting, stopWaiting, submitFailure, detectErrorResponse }  from '@willsofts/will-app5';
import { confirmUpdate, confirmSave, confirmDelete, successbox, serializeParameters } from '@willsofts/will-app5';
import { replaceString } from "@willsofts/will-app5";
import { InputDate, InputNumber, InputMoney, InputMask } from '@willsofts/will-control';
...
export default {
  components: {
    DialogForm, InputDate, InputNumber, InputMoney, InputMask
  },
  props: {
    modes: Object,
    labels: Object,
  ...
  methods: {
    ...
    showDialog(callback) {
      ...
      if(callback) this.$refs.dialogForm.$el.addEventListener("shown.bs.modal",callback);
