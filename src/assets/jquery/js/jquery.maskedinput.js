		...
		mask: function(mask, settings) {
			...
			return this.each(function() {
				...
				function keypressEvent(e) {
					...
					if (e.ctrlKey || e.altKey || e.metaKey) {//Ignore
						...
						if(checkLength()||((pos.begin-pos.end)!=0)) {
							if (p < len) {								
								if($input.val().length==0) buffer = $.map(mask.split(""), function(c) { if (c != '?') return defs[c] ? settings.placeholder : c });
				...
				function clearBuffer(start, end) {					
					...
					if(checkLetter()) {
						var j = 0;
						var temp = $.map(mask.split(""), function(c) { if (c != '?') return defs[c] ? settings.placeholder : c });
